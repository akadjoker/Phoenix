...
void CheckGLError(const char* location) {
    ...
    while ((err = glGetError()) != GL_NO_ERROR) {
        ...
        printf("❌ OpenGL Error at [%s]: %s (0x%X)\n", location, errorStr, err);
    }
...
{
private:
    // HDR framebuffer (cena principal)
    GLuint hdrFBO;
    GLuint hdrColorBuffer; // Float texture
    GLuint hdrDepthBuffer;

    // Bright pass (extração de brilho)
    GLuint brightFBO;
...
public:
    ...
    {
        // ============================================
        // 1. HDR FRAMEBUFFER (cena principal)
        // ============================================
        glGenFramebuffers(1, &hdrFBO);
        glBindFramebuffer(GL_FRAMEBUFFER, hdrFBO);

        // HDR color buffer (RGBA16F para suportar valores > 1.0)
        ...
        glFramebufferRenderbuffer(GL_FRAMEBUFFER, GL_DEPTH_ATTACHMENT,
                                  GL_RENDERBUFFER, hdrDepthBuffer);

        if (glCheckFramebufferStatus(GL_FRAMEBUFFER) != GL_FRAMEBUFFER_COMPLETE)
            LogError("ERROR: HDR Framebuffer not complete!\n");

        // ============================================
        // 2. BRIGHT PASS FRAMEBUFFER (extração)
        // ============================================
        glGenFramebuffers(1, &brightFBO);
        CHECK_GL_ERROR(glBindFramebuffer(GL_FRAMEBUFFER, brightFBO));

        glGenTextures(1, &brightTexture);
        CHECK_GL_ERROR(glBindTexture(GL_TEXTURE_2D, brightTexture));
        glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA16F,
                     screenWidth / 2, screenHeight / 2, // Metade resolução
                     0, GL_RGBA, GL_FLOAT, nullptr);
        glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
        glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
        glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE);
        ...
        if (glCheckFramebufferStatus(GL_FRAMEBUFFER) != GL_FRAMEBUFFER_COMPLETE)
            LogError("ERROR: Bright Pass Framebuffer not complete!\n");

        // ============================================
        // 3. PING-PONG BUFFERS (blur)
        // ============================================
        ...
        for (int i = 0; i < 2; i++)
        {
            CHECK_GL_ERROR(glBindFramebuffer(GL_FRAMEBUFFER, pingpongFBO[i]));
            CHECK_GL_ERROR(glBindTexture(GL_TEXTURE_2D, pingpongBuffer[i]));
            glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA16F,
            ...
            if (glCheckFramebufferStatus(GL_FRAMEBUFFER) != GL_FRAMEBUFFER_COMPLETE)
                LogError("ERROR: Ping-Pong Framebuffer not complete!\n");
        }

        CHECK_GL_ERROR(glBindFramebuffer(GL_FRAMEBUFFER, 0));
    }

    void Render(Shader *brightShader, Shader *blurShader, GLuint sceneTexture, QuadRenderer &quadRenderer)
    {
        ...
        brightShader->Bind();
        brightShader->SetUniform("threshold", 2.5f); // Brightness threshold
        ...
        for (int i = 0; i < blurAmount; i++)
        {
            ...
            glBindTexture(GL_TEXTURE_2D,
                          i == 0 ? brightTexture : pingpongBuffer[!horizontal]);
    ...
    GLuint GetHDRTexture() const { return hdrColorBuffer; } 
    GLuint GetBlurredTexture() const { return pingpongBuffer[0]; }

    GLuint GetBrightTexture() const { return brightTexture; }
    GLuint GetBrightFBO() const { return brightFBO; }
};
...
// ============================================
// BRIGHT PASS SHADER (Extração de brilho)
// ============================================
const char *brightPassVertexShader = R"(
#version 300 es
precision highp float;
layout (location = 0) in vec3 aPos;
layout (location = 1) in vec2 aTexCoords;
...
const char *brightPassFragmentShader = R"(
#version 300 es
precision highp float;
out vec4 FragColor;
...
void main()
{
    ...
    {
        //  OPCIONAL: Intensificar o brilho extraído
        FragColor = vec4(color * 0.5, 1.0);  // Multiplicar para realçar
    }
    else
    {
...
// ============================================
// FINAL COMPOSITE SHADER (HDR + Bloom + Tone Mapping)
// ============================================
...
void main()
{
    ...
    //  Adicionar bloom
    hdrColor += bloomColor * 0.95;  // bloomStrength ~0.3 a 0.8
    
    //  Tone mapping (Reinhard)
    vec3 result = hdrColor / (hdrColor + vec3(1.0));
    
    // OU usar exposure-based:
    // vec3 result = vec3(1.0) - exp(-hdrColor * 0.99);
...
int main()
{
    ...
    Shader *blurShader = ShaderManager::Instance().Create("blur", brightPassVertexShader, blurFragmentShader);
    Shader *compositeShader = ShaderManager::Instance().Create("composite", brightPassVertexShader, compositeFragmentShader);
    Shader *brightPassShader = ShaderManager::Instance().Create("brightPass", brightPassVertexShader, brightPassFragmentShader);

    BloomRenderer bloomRenderer(screenWidth, screenHeight);
    ...
    float bloomStrength = 0.15f;   // No composite
    float exposure = 0.85f;        // No composite
    ...
    while (device.IsRunning())
    {
       ...
       if (state[SDL_SCANCODE_U])
        ...
        Mat4 model;
        float S = 0.2f;
        Mat4 matModel = Mat4::Translation(5.4f, 0.9f, -0.5f) * Mat4::Scale(S, S, S);
        ...
        glBindFramebuffer(GL_FRAMEBUFFER, bloomRenderer.GetHDRFBO());
        glViewport(0, 0, screenWidth, screenHeight);
        ...
        // PASS 2: BLOOM PROCESS

        bloomRenderer.Render(brightPassShader, blurShader, bloomRenderer.GetHDRTexture(), quad);
         CheckGLError("Gen HDR FBO");
...
// debugShader->Bind();
// glActiveTexture(GL_TEXTURE0);
// glBindTexture(GL_TEXTURE_2D, bloomRenderer.GetBrightTexture());   
// debugShader->SetUniform("depthMap", 0);
// quad.render();
//     CheckGLError("Bind HDR FBO");
        // PASS 3: Composite final
   
        glBindFramebuffer(GL_FRAMEBUFFER, 0); // Render normal
        glViewport(0, 0, screenWidth, screenHeight);
        glClear(GL_COLOR_BUFFER_BIT);

        compositeShader->Bind();
        compositeShader->SetUniform("bloomStrength", bloomStrength);
        compositeShader->SetUniform("exposure",exposure);

        // Textura HDR original
        glActiveTexture(GL_TEXTURE0);
        glBindTexture(GL_TEXTURE_2D, bloomRenderer.GetHDRTexture()); // ← HDR original
        compositeShader->SetUniform("hdrBuffer", 0);

        // Textura do bloom (blurred)
        glActiveTexture(GL_TEXTURE1);
        glBindTexture(GL_TEXTURE_2D, bloomRenderer.GetBlurredTexture()); // ← Bloom
        ...
        font.Print(10, Y * 2, " [I/U]  Bloom :%f", bloomStrength);

        font.Print(10, Y * 3, " [P/O]   Exposure :%f", exposure);


        font.Print(10, Y * 4, " Light pos :%f %f %f", lightPos.x, lightPos.y, lightPos.z);
