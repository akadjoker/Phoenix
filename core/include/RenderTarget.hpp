...
private:
    ...
    std::vector<unsigned int> dummyColors;
    ...
    Mat4 CalculateLightSpaceMatrix(float nearPlane, float farPlane,
                                   const Mat4 &proj, const Mat4 &view,
                                   const Vec3 &lightDir);

public:
    ShadowMapManager(u32 shadowWidth = 2048, u32 shadowHeight = 2048, int cascadeCount = 4);
    ~ShadowMapManager();
    ...
    void Calculate(const Mat4 &view, const Mat4 &proj, float nearPlane, float farPlane, const Vec3 &lightDir);

    void BeginDepthPass();
    void BindCascade(int cascade);
    void EndDepthPass();

    void BindShadowMapsToShader(Shader *shader, int startTextureUnit = 1);
    void SetShaderUniforms(Shader *shader);

    const Mat4 &GetLightSpaceMatrix(int cascade) const;
    float GetCascadeSplit(int cascade) const;
    u32 GetDepthMap(int cascade) const;
