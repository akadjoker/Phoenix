...
private:
    ...
    float m_smoothingFactor = 0.15f; // 0.0 = sem smooth, 1.0 = mudança instantânea
    Vec3 m_desiredVelocity; // Velocidade alvo suavizada

    Quat m_modelForwardOffset;
    float m_rotationSmoothFactor = 0.1f; 

    
    float m_rollAngle = 90.0f;
    
    bool computeDesiredOrientation(Quat& outRotation, Vec3& outForward) const;
...
public:
    ...
    void setSmoothingFactor(float factor); // 0.05-0.3 é bom range
    ...
    Vec3 pursue(const Vec3& targetPos, const Vec3& targetVelocity, float weight = 1.0f);
    Vec3 evade (const Vec3& targetPos, const Vec3& targetVelocity, float weight = 1.0f);
    ...
    Vec3 align(const std::vector<GameObject *> &neighbors,
               float neighborRadius, float maxSpeed = -1.0f);
