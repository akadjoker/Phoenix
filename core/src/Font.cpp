...
// Convert Pixmap components to TextureFormat
TextureFormat PixmapToTextureFormat(int components)
{
    switch (components)
    {
        case 1: return TextureFormat::R8;      // Grayscale
        case 2: return TextureFormat::RG8;     // Grayscale + Alpha
        case 3: return TextureFormat::RGB8;    // RGB
        case 4: return TextureFormat::RGBA8;   // RGBA
        default: 
            LogWarning("Unknown component count: %d, defaulting to RGBA8", components);
...
//     texture->Create(pixmap.width, pixmap.height, TextureFormat::RG8, pixmap.pixels);
...
//     LogInfo("[FONT]: Default font loaded successfully (%i glyphs)",
//             m_glyphCount);
...
bool Font::LoadDefaultFont()
{
    ...
    // Create texture
    texture = new Texture();
    if (!texture->Create(pixmap.width, pixmap.height, TextureFormat::RG8, pixmap.pixels))
    {
        LogError("[Font] Failed to create texture for default font");
        delete texture;
        texture = nullptr;
        return false;
    }
...
bool Font::LoadBMFont(const char* fntPath, const char* texturePath)
{
   ...
   // int scaleW = 0, scaleH = 0;
    ...
    TextureFormat format = PixmapToTextureFormat(pixmap.components);
    if (!texture->Create(pixmap.width, pixmap.height, format, pixmap.pixels))
    {
        LogError("[Font] Failed to create texture from pixmap");
        delete texture;
        texture = nullptr;
        return false;
    }
                         ...
                         const char* charset)
{
    ...
    if (!texture->Create(pixmap.width, pixmap.height, format, pixmap.pixels))
    {
        LogError("[Font] Failed to create texture");
        delete texture;
        texture = nullptr;
        return false;
    }
...
bool Font::LoadTTF(const char* ttfPath, float size, int atlasSize)
{
    ...
    if (!file.is_open())
    {
        LogError("[Font] Failed to open TTF file: %s", ttfPath);
        return false;
    }
    ...
    {
        LogError("[Font] Failed to read TTF file");
        return false;
    }
                            ...
                            float size, int atlasSize)
{
    ...
    LogInfo("[Font] TTF font loaded (%.0fpx, %d glyphs)", size, m_glyphCount);
...
{
    LogError("[Font] TTF loading not available - compile with USE_STB_TRUETYPE");
    return false;
}
