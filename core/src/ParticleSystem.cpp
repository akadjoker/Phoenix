...
ParticleSystem::ParticleSystem(const std::string& name, int maxParticles)
    : Node3D(name)
    // particles (vector - não precisa inicializar)
    // affectors (vector - não precisa inicializar)
    // vertices (vector - não precisa inicializar)
    // indices (vector - não precisa inicializar)
    , vao(nullptr)
    , vb(nullptr)
    , ib(nullptr)
    , texture(nullptr)
    ...
    , pulseRate(1.0f)
    , particlesPerPulse(5)
    // atlasFrames (vector - não precisa inicializar)
    , useAtlas(false)
    , currentFrame(0)
    , radius(1.0f)
    , innerRadius(0.5f)
