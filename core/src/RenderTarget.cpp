...
bool RenderTarget::Finalize()
{
    ...
    LogInfo("[RenderTarget:%s] Framebuffer complete!", m_name.c_str());
    ...
    if (m_hasDepth)
    {
        ...
        {
            LogInfo("  Depth: Texture ID %u", m_depthAttachment.texture->GetHandle());
        }
        else if (m_depthAttachment.rbo)
        {
            LogInfo("  Depth: Renderbuffer ID %u", m_depthAttachment.rbo);
        }
    }
    ...
    for (size_t i = 0; i < m_colorAttachments.size(); i++)
    {
        ...
        {
            LogInfo("  Color[%zu]: Texture ID %u", i, m_colorAttachments[i].texture->GetHandle());
        }
    }
...
bool MSAARenderTarget::AddDepthAttachment(TextureFormat format)
{
    ...
    GLenum attachment = (format == TextureFormat::DEPTH24_STENCIL8) ? GL_DEPTH_STENCIL_ATTACHMENT : GL_DEPTH_ATTACHMENT;
