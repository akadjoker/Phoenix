...
int main()
{
    ...
    Texture *texture = TextureManager::Instance().Add("wall.jpg", true);
    ...
    //Mesh *room = MeshManager::Instance().Import("room", "assets/room.3ds");
     ...
     Mesh *room = MeshManager::Instance().Load("room", "assets/sponza.h3d");
   //  room->CalculateNormals();
     ...
     while (device.Run())
    {
        ...
        // const Uint8 *state = SDL_GetKeyboardState(NULL);
         ...
         if (Input::IsKeyDown(KEY_KP_SUBTRACT))
             ssao.ssaoPower += 0.1f * dt * 10.0f;
         if (Input::IsKeyDown(KEY_KP_ADD))
             ssao.ssaoPower -= 0.1f * dt * 10.0f;

        if (Input::IsKeyDown(KEY_KP_8))
            ssao.ssaoRadius += 0.05f * dt * 10.0f;
        if (Input::IsKeyDown(KEY_KP_2))
            ssao.ssaoRadius -= 0.05f * dt * 10.0f;
             

        
        if (Input::IsKeyDown(KEY_KP_4))
            ssao.ssaoBias += 0.001f * dt * 10.0f;
        if (Input::IsKeyDown(KEY_KP_6))
            ssao.ssaoBias -= 0.001f * dt * 10.0f;

        if (Input::IsKeyDown(KEY_KP_2))
            ssao.ambientStrength += 0.05f * dt * 10.0f;
        if (Input::IsKeyDown(KEY_KP_3))
            ssao.ambientStrength -= 0.05f * dt * 10.0f;
